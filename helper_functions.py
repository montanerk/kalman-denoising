...
def sliding_window(signal,Fs):
    ...
    # Reshape
    signal_sliced_half_stationary = np.reshape(np.concatenate((signal_padding, np.zeros((half_stationary,1)))),(half_stationary,-1), order='F') # recouvrement 50%
    
    signal_sliced_half_stationary_delayed =  np.reshape(np.concatenate((np.zeros((half_stationary,1)),signal_padding)),(half_stationary,-1), order='F') # recouvrement 50%
    
    signal_sliced = np.concatenate((signal_sliced_half_stationary_delayed,signal_sliced_half_stationary),axis=0)
    
    signal_sliced = signal_sliced[:,1:-1] # Enleve ce qu'on a ajoute artifitiallement
